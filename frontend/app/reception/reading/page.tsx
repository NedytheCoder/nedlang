...
export default function ReadingPage() {
  ...
  // Fake progress ticker during loading
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/user/profile/${userId}`)
      ...
      .then((profile) => {
        ...
        return fetch(`${BACKEND_URL}/reception/test/reading_questions`, {
          method: "POST",
  ...
  if (pageState === "loading") {
    return <AssessmentLoader progress={Math.min(loadProgress, 100)} />
  }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="relative z-10 max-w-xl mx-auto px-4 sm:px-6 py-8">
        ...
        >
          ...
          {/* Question area */}
          <div className="px-5 sm:px-8 py-6 min-h-[360px]">
          ...
          {!isDone && (
            <div className="px-5 sm:px-8 pb-6 pt-2 border-t border-slate-100 dark:border-white/6">
              ...
              >
                {isFinal ? t("assess_continue_listening") : t("assess_submit")}
              </motion.button>
            </div>
          )}
        </motion.div>
